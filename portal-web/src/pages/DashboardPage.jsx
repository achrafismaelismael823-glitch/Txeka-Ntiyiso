...
const StatCard = ({ title, value, subtitle, icon: Icon, color, trend, onClick }) => (
  <div 
    onClick={onClick}
    className={`relative glass-panel p-5 cursor-pointer group hover:bg-[#112240]/60 transition-all duration-300 ${onClick ? 'hover:-translate-y-0.5 hover:shadow-lg' : ''}`}
  >
    ...
    <div className="flex items-start justify-between">
      ...
      <div className="p-2.5 rounded-xl bg-white/[0.03] border border-white/[0.06] group-hover:bg-white/[0.06] transition-colors">
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        if (isAdmin) {
          ...
          // InstitutionListResponse: { total, institutions }
          ...
          // InstitutionDashboard: { institution, credits_history, total_emitted, total_verifications }
          const dashboard = dashboardRes.status === 'fulfilled' ? dashboardRes.data : {};
          
          // InstitutionCredits: { credits, status, docs_emitted_month }
          const credits = creditsRes.status === 'fulfilled' ? creditsRes.data : {};

          setStats({
            documentsEmitted: dashboard.total_emitted || 0,
            documentsVerified: dashboard.total_verifications || 0,
            creditsBalance: credits.credits || 0,
            docsEmittedMonth: credits.docs_emitted_month || 0,
            creditStatus: credits.status || 'active',
          });

          // Usa credits_history do dashboard como atividade recente
        ...
        setStats(isAdmin ? {
          ...
          docsEmittedMonth: 0, creditStatus: 'active',
        });
      } finally {
        setLoading(false);
      }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <h2 className="text-sm font-bold text-[#f8fafc] uppercase tracking-wider flex items-center gap-2">
            <Zap className="w-4 h-4 text-[#06b6d4]" />Ações Rápidas
          </h2>
          ...
          <div className="mt-6">
            <h2 className="text-sm font-bold text-[#f8fafc] uppercase tracking-wider flex items-center gap-2 mb-4">
              ...
              {isAdmin ? 'Logs de Auditoria' : 'Histórico de Créditos'}
            </h2>
                  ...
                  {recentLogs.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-4 px-5 py-3.5 hover:bg-white/[0.02] transition-colors">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                        ...
                        {item.type === 'manual_add' ? <TrendingUp className="w-4 h-4" /> :
                         item.type === 'bonus' ? <Zap className="w-4 h-4" /> :
                         item.type === 'refund' ? <CreditCard className="w-4 h-4" /> :
                         <Hash className="w-4 h-4" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-[#f8fafc] truncate">
                          {item.description || `Transação de ${item.amount} créditos`}
                        </p>
        ...
        <div className="space-y-6">
          <div className="glass-panel p-5 space-y-4">
            <h3 className="text-sm font-bold text-[#f8fafc] uppercase tracking-wider flex items-center gap-2">
              <Globe className="w-4 h-4 text-[#06b6d4]" />{isAdmin ? 'Status da Rede' : 'Status da Conta'}
            </h3>
            <div className="space-y-3">
              ...
              {!isAdmin && (
                <>
                  ...
                  <div className="flex items-center justify-between text-sm"><span className="text-silver-dark/60">Plano</span><span className="text-[0.75rem] text-[#22d3ee] font-semibold uppercase">{user?.subscription_plan || 'standard'}</span></div>
                </>
              )}
            </div>
            ...
            <div className="p-3 rounded-lg bg-white/[0.02] border border-white/[0.04]">
              <p className="text-[0.65rem] text-silver-dark/40 leading-relaxed text-center">
                {isAdmin ? 'Modo administrativo ativo. Todas as alterações são registadas em auditoria.' : 'Acesso restrito aos recursos da sua instituição.'}
              </p>
            </div>
          </div>
