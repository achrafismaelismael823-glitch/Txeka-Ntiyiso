...
// ── QUICK ACTION CARD ──
const QuickAction = ({ icon: Icon, label, description, to, color, onClick }) => {
  ...
  <div className="col-span-full p-8 rounded-2xl bg-slate-900/40 border border-white/[0.06] text-center space-y-4">
    ...
    <div>
      ...
      <p className="text-xs text-slate-600 mt-1 max-w-sm mx-auto">
        A dashboard será preenchida automaticamente à medida que documentos forem emitidos e verificados na plataforma.
      </p>
    </div>
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <QuickAction
          ...
          description="Certifique um novo documento digital"
          to="/emit"
        ...
        <QuickAction
          ...
          description="Valide a autenticidade por hash"
          to="/verify"
        ...
        <QuickAction
          icon={FileText}
          label="Documentos"
          description="Consulte o histórico de emissões"
          to="/documents"
      ...
      {/* Top Institutions (Admin only) */}
      {isAdmin && (stats?.top_institutions?.length > 0 || loading) && (
        <div className="p-5 rounded-2xl bg-slate-900/60 backdrop-blur-xl border border-white/[0.06] space-y-4">
          <div className="flex items-center gap-2 text-xs text-slate-500 uppercase tracking-wider">
