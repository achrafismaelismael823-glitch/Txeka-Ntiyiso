...
import { ShieldCheck, Lock, Mail, Eye, EyeOff, AlertCircle, WifiOff, ServerOff } from 'lucide-react';
        ...
        <div className="bg-white rounded-[2rem] p-8 shadow-2xl">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">ID da Instituicao</label>
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full bg-slate-50 border text-slate-900 text-sm rounded-2xl pl-12 pr-4 py-4 focus:outline-none transition ${error?.code === 'VALIDATION' && error.fields?.some(f => f.includes('ID')) ? 'border-rose-400 focus:border-rose-500' : 'border-slate-200 focus:border-[#00D2C4]'}`}
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  disabled={isLocked || isLoading}
                  className={`w-full bg-slate-50 border text-slate-900 text-sm rounded-2xl pl-12 pr-12 py-4 focus:outline-none transition ${error?.code === 'VALIDATION' && error.fields?.some(f => f.includes('Senha')) ? 'border-rose-400 focus:border-rose-500' : 'border-slate-200 focus:border-[#00D2C4]'}`}
                />
