    ...
    return (
      <div className="min-h-screen bg-[#0B192C] flex flex-col items-center justify-center font-sans">
        <div className="text-center space-y-4">
          ...
          <p className="text-xs font-mono tracking-widest text-[#00D2C4] uppercase">A inicializar canal seguro...</p>
        </div>
      </div>
    );
  }
...
function RedirectRoot() {
  ...
  useEffect(() => { setIsAuth(isAuthenticated()); }, []);
      ...
      <Route path="*" element={
        <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC] font-sans p-4">
          <div className="text-center max-w-sm w-full bg-white border border-slate-200 p-6 rounded-2xl shadow-sm">
            ...
            <p className="text-sm font-medium text-slate-600 mt-1 mb-5">O recurso ou ecra solicitado nao existe na rede.</p>
            <a href="/" className="block w-full bg-[#0B192C] hover:bg-slate-800 text-white font-semibold text-sm py-3 px-4 rounded-xl transition shadow-sm">
