...
class EmissionService:
    ...
    async def certify_document(self, institution_id: str, document_type: str, file_name: str, content: str, issued_by: str) -> Dict[str, Any]:
        ...
        document = Document(doc_id=doc_id, doc_hash=hash_sha256, document_type=document_type, institution_id=institution_id, file_name=file_name, issued_by=issued_by, certificate_url=f"https://verify.txekantiyiso.gov.mz/{doc_id}", qr_code=f"QR_{doc_id}")
        institution.credits -= 1
        institution.docs_emitted_month += 1
        self.db.add(document)
        return {"doc_id": document.doc_id, "certificate_url": document.certificate_url, "credits_remaining": institution.credits, "hash_sha256": hash_sha256}
    ...
    async def certify_bulk_documents(self, institution_id: str, documents_list: List[Dict[str, Any]], issued_by: str) -> Dict[str, Any]:
        ...
        if institution.credits < total_docs:
            raise TxekaNtiyisoException(message=f"Creditos insuficientes para lote. Necessita de {total_docs}, mas possui {institution.credits}.", status_code=402)
        processed_docs = []
        for doc_data in documents_list:
            ...
            new_doc = Document(doc_id=doc_id, doc_hash=hash_sha256, document_type=doc_data.get("document_type", "DOC"), institution_id=institution_id, file_name=doc_data.get("file_name"), issued_by=issued_by, certificate_url=f"https://verify.txekantiyiso.gov.mz/{doc_id}", qr_code=f"QR_{doc_id}")
            self.db.add(new_doc)
        ...
        return {"message": f"Lote de {total_docs} documentos processado com sucesso.", "documents": processed_docs, "credits_remaining": institution.credits}
